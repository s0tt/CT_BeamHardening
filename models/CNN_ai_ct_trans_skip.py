...
class CNN_AICT_TRANS_SKIP(pl.LightningModule):
    ...
    def __init__(self, ref_img=None, plot_test_step=None, plot_val_step=None, plot_weights=False, custom_init=False):
        ...
        self.linearEndLayer = nn.Sequential(
            nn.Linear(256, 256),
            nn.Sigmoid(),
            nn.Linear(256, 256)
        )
    ...
    def forward(self, x):
        ...
        out = self.linearStartLayer(x)
        out_start = self.startLayer(out)
        out = self.middleLayer(out_start)
        out += out_start
        out = self.endLayer(out)
        out = self.linearEndLayer(out)
